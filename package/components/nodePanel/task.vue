      ...
      return {
        ...
        item: [
          ...
          },
		  {
		    ...
		    activeText: '是',
		    inactiveText: '否',
		    show: !!_this.showConfig.taskType
		  },
		  // {
		  //   xType: 'input',
		  //   type: 'hidden',
  ...
  watch: {
    'formData.userType': function(val, oldVal) {
      if (oldVal) {
        ...
        types.forEach(type => {
          delete this.element.businessObject.$attrs[`camunda:${type}`]
    ...
    'formData.assignee': function(val) {
      if (this.formData.userType !== 'assignee') {
        delete this.element.businessObject.$attrs[`camunda:assignee`]
        return
      }
      this.updateProperties({ 'camunda:assignee': val })
    },
    'formData.candidateUsers': function(val) {
      if (this.formData.userType !== 'candidateUsers') {
        delete this.element.businessObject.$attrs[`camunda:candidateUsers`]
        return
      }
      this.updateProperties({ 'camunda:candidateUsers': val?.join(',') })
    },
    'formData.candidateGroups': function(val) {
      if (this.formData.userType !== 'candidateGroups') {
        delete this.element.businessObject.$attrs[`camunda:candidateGroups`]
        return
      }
      this.updateProperties({ 'camunda:candidateGroups': val?.join(',') })
    },
    'formData.taskType': function(val) {
      if (val === '') val = null
	  console.log(this.formData)
		let documentation = JSON.parse(this.formData.documentation);
		documentation.type = val?1:0;
		this.formData.documentation = JSON.stringify(documentation);
      this.updateProperties({ 'camunda:taskType': true })
	 },
	'formData.async': function(val) {
	  if (val === '') val = null
	  this.updateProperties({ 'camunda:async': true })
	},
	
    'formData.dueDate': function(val) {
      if (val === '') val = null
      this.updateProperties({ 'camunda:dueDate': val })
    },
    ...
    'formData.triggerable': function(val) {
      if (val === '') val = null
      this.updateProperties({ 'camunda:triggerable': val })
    },
    'formData.class': function(val) {
      if (val === '') val = null
      this.updateProperties({ 'camunda:class': val })
    },
    'formData.autoStoreVariables': function(val) {
      if (val === '') val = null
      this.updateProperties({ 'camunda:autoStoreVariables': val })
    },
    'formData.exclude': function(val) {
      if (val === '') val = null
      this.updateProperties({ 'camunda:exclude': val })
    },
    'formData.ruleVariablesInput': function(val) {
      if (val === '') val = null
      this.updateProperties({ 'camunda:ruleVariablesInput': val })
    },
    'formData.rules': function(val) {
      if (val === '') val = null
      this.updateProperties({ 'camunda:rules': val })
    },
    'formData.resultVariable': function(val) {
      if (val === '') val = null
      this.updateProperties({ 'camunda:resultVariable': val })
    }
  },
  created() {
    const cache = {
